...
SALKEN ALL` // Ensure 'prefix' is defined
                                }),
                            },
                        ],
                    }),
    ...
    messageContent,
    {
        userJid: from // gunakan 'from' sebagai userJid
    }
);


    // Send the message
    await conn.relayMessage(from, msgs.message, { messageId: msgs.key.id });
};

const sendButReply = async (text) => {
    const { proto, generateWAMessageFromContent } = require('baileys');
...
${sWelcome}`;
    ...
    const welcomeText = (chat.sWelcome || teks)
        ...
        .replace("@desc", groupDesc.toString() || "unknown")
        .replace("@subject", groupName);
        
    if (chat.welcome && !itsMe && oneMem) {
        const stikWlc = await conn.sendMessage(from, { sticker: { url: stikWelcome } });
        sendButCopy(welcomeText)
        //conn.sendMessage(from, { text: welcomeText, quoted: stikWlc });
    }
}
break;
...
let teks = `Selamat tinggal @user\n${sBye}`;
   ...
   sendButReply(byeText)
       // await conn.sendMessage(from, { text: byeText });
    }
}
break;
