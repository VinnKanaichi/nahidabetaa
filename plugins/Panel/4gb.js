...
const handler = async (m, { text, setReply, hanz,isResPanel }) => {
    ...
    let name = username + "4GB"; // Updated to reflect the 4GB name
    let egg = global.eggs;
    let loc = global.location;
    let memo = "4096"; // Memory in MB
    let cpu = "125"; // CPU limit
    let disk = "4000"; // Disk space
    ...
    let ctffff = `🌟 *SELAMAT DATANG di Panel Kami!* 🌟\n\n` +
                 `🔑 *Detail Akun Anda:* \n` +
    ...
    await hanz.sendMessage(u, { image: { url: akunlo }, caption: ctffff });

    // Mengirim pesan sukses ke pengirim perintah
    let successMessage = "✅ *Proses berhasil! Akun 4GB telah dibuat.*";
    await hanz.sendMessage(m.chat, { text: successMessage });
};

// Daftar command dan pengaturan plugin
handler.command = ['4gb']; // Ganti dengan command yang sesuai
