let handler = async (m, { hanz, sendButDoc}) => {
	let ktaBjk = `${gris1}"${pickRandom(global.quotes)}"${gris1}`
sendButDoc(ktaBjk,'Next','katabijak')
    }
handler.help = ["quotes", "katabijak"];
handler.tags = ["quotes"];
handler.command = /^(quotesbijak|katabijak)$/i;
handler.limit = true
