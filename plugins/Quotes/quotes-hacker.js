let handler = async (m, { hanz, usedPrefix, command, sendButDoc }) => {
	let hekerr = `${gris1}${pickRandom(global.heker)}${gris1}`
	sendButDoc(hekerr,'Next','quoteshacker')
};
handler.help = ["katahacker"];
handler.tags = ["quotes"];
handler.command = /^(quoteshacker|katahacker|katahekel)$/i;
handler.limit = true
