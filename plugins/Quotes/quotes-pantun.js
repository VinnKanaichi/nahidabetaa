...
let handler = async (m, { hanz, usedPrefix, __dirname, text, command, sendButDoc }) => {
let tag = `@${m.sender.replace(/@.+/, '')}`
  let mentionedJid = [m.sender]
let name = hanz.getName(m.sender)
let cin = `${gris1}${pickRandom(global.pantun)}${gris1}`
sendButDoc(cin,'Next','quotespantun')
}
handler.help = ['pantun']
handler.tags = ['quotes']
handler.command = /^(quotespantun)$/i
handler.limit = true
