...
let handler = async (m, { command, hanz, sendButImg}) => {
    ...
    for (const [keyword, url] of Object.entries(commandMap)) {
        if (new RegExp(keyword).test(command)) {
            ...
            sendButImg(`_${command}_`,yeha,'Next_',`${command}`)
...
handler.tags = ['anime', 'manga', 'fun']; // Add more tags as needed
handler.help = ['kartun', 'katakata', 'kpop', 'kucing']; // Add more help commands as needed
handler.group = true; // Change to false if you want this to work in private chats
