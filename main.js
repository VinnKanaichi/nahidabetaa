...
if(sampah) fs.unlinkSync(file)
    ...
    font: 'chrome', // Menggunakan font yang sederhana
    align: 'center',
    colors: ['cyan', 'white'], // Warna minimalis
...
const creatorInfo = [
    { label: 'Created By', value: 'Ehanzdhonax' },
    { label: 'WhatsApp', value: '6281316643491' },
    { label: 'From', value: 'Tasikmalaya' }
];

creatorInfo.forEach(info => {
    console.log(`${color(`${info.label}`, 'blue')}${color(':', 'white')} ${color(info.value, 'lightyellow')}`);
});

console.log(color(`─────────────────────────────`, 'white'));
console.log(color(`Menhera-MD is running...`, 'green'));
...
connectToWhatsApp(); 
    ...
    console.log(`${color(` `, 'white')}${color(`🎐`, 'blue')}${color(`-`, 'white')}`, `WA v${version.join('.')}`);
    console.log(`${color(` `, 'white')}${color(`🎐`, 'blue')}${color(`-`, 'blue')}`, `${calender}`);
    
    console.log(color(`]─`, 'blue'), `「`, color(`RANGELOFFICIAL`, 'white'), `」`, color(`─[`, 'blue'));
